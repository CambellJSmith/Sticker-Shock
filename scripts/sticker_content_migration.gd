...
func _init() -> void: # Performs the migration before gameplay systems load their persistent state.
	...
	_clear_book_stickers() # Removes placed and pending individual stickers while preserving book structure and navigation state.
	_clear_owned_stickers() # Removes owned individual sticker records while preserving currency and free-pack timing.
	_write_migration_marker() # Prevents the cleanup from running again after users add new custom sticker content.

func _clear_book_stickers() -> void: # Clears individual sticker instances from the persistent book save without removing book-system data.
	if not FileAccess.file_exists(BOOK_SAVE_PATH): # Skips cleanup when no prior book save exists.
		return # Leaves first-run users to receive the normal empty book save from the existing state system.
	...
	if save_file == null: # Detects a read failure without risking replacement of inaccessible user data.
		return # Leaves the save untouched when it cannot be read safely.
	...
	if parsed_data is not Dictionary: # Rejects malformed saves that the normal book loader already knows how to recover from.
		return # Avoids writing over invalid content during this narrowly scoped migration.
	...
	var write_file: FileAccess = FileAccess.open(BOOK_SAVE_PATH, FileAccess.WRITE) # Opens the same save for atomic-style complete replacement.
	if write_file == null: # Detects a write failure before serialization.
		return # Leaves the in-memory migration unapplied when the file cannot be written.
...
func _clear_owned_stickers() -> void: # Clears individual sticker ownership entries without resetting unrelated economy progression.
	if not FileAccess.file_exists(ECONOMY_SAVE_PATH): # Skips cleanup when no prior economy save exists.
		return # Leaves first-run users to receive the normal empty ownership save from the existing economy system.
