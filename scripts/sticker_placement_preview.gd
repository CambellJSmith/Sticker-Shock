...
func configure(sticker_size: Vector2, sticker_texture: Texture2D) -> void: # Builds the floating physical sticker and its exact page-projected landing silhouette.
	...
	_visual.configure(sticker_size, sticker_texture) # Builds the printed front, reverse, and physical sheet geometry from the supplied artwork alpha.
...
func clear() -> void: # Removes every manual placement helper before returning to the shop or converting the preview into a real sticker.
