class_name CollectionItem extends GameButton # Makes each normal or premium collection card a native placement action.
...
var _sticker_key: String = "" # Retains the stable normal or premium per-copy identity used by inventory and placement.
...
func configure(sticker_key: String, texture: Texture2D, place: Callable) -> void: # Initializes this reusable card once per exact collection edition entry.
	_sticker_key = sticker_key # Stores the edition-aware identity independently of its artwork resource path.
	var artwork_path: String = StickerVariant.get_art_path(sticker_key) # Resolves the authored artwork used by every finish.
	...
	var edition_name: String = StickerVariant.get_edition_name(sticker_key) # Reads the exact normal, rainbow, silver, or gold finish.
	_search_name = ("%s %s" % [edition_name, base_name]) if StickerVariant.is_special(sticker_key) else base_name # Makes premium entries searchable by finish while preserving base-name matches.
	_name_label.text = ("★ %s · %s" % [edition_name, base_name]) if StickerVariant.is_special(sticker_key) else base_name # Labels each premium copy as a distinct collection entry.
...
func get_sticker_key() -> String: # Exposes the stable normal or premium identity for model lookups.
