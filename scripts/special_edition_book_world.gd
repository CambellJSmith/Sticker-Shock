...
func begin_manual_placement(sticker_key: String, pending_index: int) -> bool: # Enters book placement mode for one exact normal or premium edition copy selected from the collection.
	...
	_manual_sticker_path = sticker_key # Preserves the normal-or-premium identity for controller validation and persistent placement.
	...
	_manual_preview.configure(_manual_sticker_size, texture_resource as Texture2D, sticker_key) # Builds the exact sheet and applies normal, rainbow, silver, or gold from the reserved copy identity.
...
func _process(delta: float) -> void: # Advances the short post-placement delay before returning to the collection that initiated placement.
	...
	if _return_to_shop_remaining <= 0.0: # Detects when the newly placed sticker has had enough time to visibly complete its landing.
		...
		_controller.show_collection() # Returns to the collection so the player can choose another available copy deliberately.
...
func _spawn_placement_record(placement: Dictionary, animate_landing: bool) -> void: # Recreates one persistent normal, rainbow, silver, or gold placement as a fully interactive physical Sticker node.
	...
	var artwork_path: String = StickerVariant.get_art_path(sticker_key) # Resolves the real imported artwork shared by all editions.
	...
	var sticker: Sticker # Stores the physical sticker node chosen for this exact copy finish.
	if StickerVariant.is_special(sticker_key): # Uses the edition-aware subclass only for rainbow, silver, or gold copies.
		var edition_sticker: SpecialEditionSticker = SpecialEditionSticker.new() # Creates the shared premium physical sticker implementation.
		edition_sticker.set_edition(StickerVariant.get_edition(sticker_key)) # Applies the exact persistent edition before material configuration.
		sticker = edition_sticker # Exposes the configured premium node through the common Sticker interface.
	else: # Keeps ordinary stickers on the original physical component.
		sticker = Sticker.new() # Creates the unchanged normal printed sticker.
	...
	sticker.configure(sticker_size, texture_resource as Texture2D) # Builds picking, peel mesh, reverse material, landing outline, and exact premium front when applicable.
	...
	_sticker_paths[runtime_id] = sticker_key # Preserves the exact edition identity so inspection can reproduce the correct material treatment.
