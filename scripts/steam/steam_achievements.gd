...
const MAX_RETRY_ATTEMPTS: int = 10 # Bounds retry work when Steamworks has not published the configured achievement API name or stats remain unavailable.
...
var _retry_attempt_count: int = 0 # Counts native unlock attempts for the current earned condition so a bad dashboard configuration cannot retry forever.
...
func _process(delta: float) -> void: # Retries an earned achievement until Steam's asynchronous stats state accepts and stores it or the bounded retry window expires.
	...
	if _first_sticker_pending and _retry_attempt_count >= MAX_RETRY_ATTEMPTS: # Detects a persistent stats or dashboard configuration failure after the bounded retry window.
		push_warning("Steam achievement %s could not be stored after %d attempts" % [String(FIRST_STICKER), MAX_RETRY_ATTEMPTS]) # Reports the exact API name so unpublished dashboard configuration is straightforward to diagnose.
...
func sync_first_sticker(total_owned_count: int) -> void: # Reconciles persisted or newly granted ownership with the first-sticker Steam achievement.
	...
	_retry_attempt_count = 0 # Starts a fresh bounded retry window for this startup reconciliation or zero-to-one ownership transition.
	...
	set_process(true) # Enables bounded retries because Steam is live but its asynchronous stats cache may not be ready yet.
