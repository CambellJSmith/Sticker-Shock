...
class StickerCreatorApp:
    ...
    def configure_style(self) -> None:
        ...
        style.configure("TButton", background=FIELD, foreground=TEXT, bordercolor=SEPARATOR, padding=(9, 4))
        style.map("TButton", background=[("active", FIELD_HOVER), ("pressed", ACCENT)])
        ...
        style.configure("Treeview", background=PANEL, foreground=TEXT, fieldbackground=PANEL, bordercolor=SEPARATOR, rowheight=24)
        style.map("Treeview", background=[("selected", SELECT)], foreground=[("selected", "#ffffff")])
        style.configure("Treeview.Heading", background=PANEL_ALT, foreground=TEXT, bordercolor=SEPARATOR, padding=(6, 4), font=("Sans", 9, "bold"))
    ...
    def build_ui(self) -> None:
        ...
        self.commit_button = ttk.Button(top_bar, text="Commit", style="Accent.TButton", command=self.commit_content)
        self.commit_button.pack(side=tk.RIGHT)
        ttk.Label(top_bar, text="PR + merge", style="Header.TLabel").pack(side=tk.RIGHT, padx=(0, 8))
    ...
    def browse_art(self) -> None:
        selected_path: str = filedialog.askopenfilename(parent=self.root, title="select sticker art", filetypes=[("PNG image", "*.png")])
        if selected_path:
    ...
    def next_sticker_id(self) -> int:
        return max(self.existing_ids(), default=0) + 1
    ...
    def commit_content_worker(self) -> None:
        try:
            result_message: str = self.perform_commit_workflow()
        except (OSError, subprocess.CalledProcessError, RuntimeError) as commit_error:
            error_text: str = str(commit_error)
            if isinstance(commit_error, subprocess.CalledProcessError):
                error_text = commit_error.stderr.strip() or commit_error.stdout.strip() or error_text
            self.root.after(0, lambda: self.finish_commit_error(error_text))
            return
        self.root.after(0, lambda: self.finish_commit_success(result_message))
    ...
    def perform_commit_workflow(self) -> str:
        ...
        all_changes: list[str] = [line for line in run_command(["git", "status", "--porcelain", "--untracked-files=all"]).stdout.splitlines() if line.strip()]
        generated_changes: list[str] = [line for line in run_command(["git", "status", "--porcelain", "--untracked-files=all", "--", *GENERATED_PATHS]).stdout.splitlines() if line.strip()]
        unrelated_count: int = len(all_changes) - len(generated_changes)
        if unrelated_count > 0:
            raise RuntimeError("There are unrelated local changes. Commit or remove those first so the sticker tool cannot include or disturb them.")
